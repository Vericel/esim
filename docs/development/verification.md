...
| 发布物使用正式 PyPI onelogg 依赖且不混入 vendor 缓存；开发环境具备 `--no-build-isolation` 所需的精确版本构建工具；wheelhouse 不覆盖非空目录 | `test_esim_wheel_preserves_the_standalone_ff_command`、`test_wheel_uses_published_onelogg_dependency`、`test_wheelhouse_builder_preserves_and_rejects_nonempty_output` |
