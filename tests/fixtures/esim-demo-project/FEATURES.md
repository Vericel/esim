...
|---|---|---|
| Rules/TC 角色及 `.yaml` | `xxx/yyy/rules/full.yaml`、`tests/features/complete.yaml` | `test_complete_yaml_demo_composes_every_configuration_layer` |
...
| unknown field 忽略与诊断 | `metadata`、`ff.timeout`、`run.enabled` | complete-yaml workspace 的 warnings/result |
...
| `continue_on_error` 合并与失败截断 | yyy full-left/complete 配置 | 配置继承测试与 execution failure 测试 |
...
执行语义、自动 cache fingerprint 和内置 waiver。demo 中的同名 unknown fields 只用于
展示“忽略并审计”，不表示支持这些未来能力。
